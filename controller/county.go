...
//MyDistrictHosListHandler 展示本区所有医院
func MyDistrictHosListHandler(c *gin.Context) {
	...
	page, size := getPageInfo(c)
	//获取数据
	data, err := logic.MyDistrictHosList(userName, page, size)
	if err != nil {
		zap.L().Error("Logic.MyDistrictHosList() failed", zap.Error(err))
		ResponseError(c, CodeServerBusy)
		return
	}
	//返回响应
...
//MyDistrictDocListHandler  展示本区所有医生
func MyDistrictDocListHandler(c *gin.Context) {
	userName, err := getCurrentUserName(c)
	if err != nil {
		zap.L().Error("GetCurrentUserName() failed", zap.Error(err))
		ResponseError(c, CodeNotLogin)
	...
	page, size := getPageInfo(c)
	//获取数据
	data, err := logic.MyDistrictDocList(userName, page, size)
	if err != nil {
		zap.L().Error("Logic.MyDistrictDocList() failed", zap.Error(err))
		ResponseError(c, CodeServerBusy)
		return
	}
	//返回响应
